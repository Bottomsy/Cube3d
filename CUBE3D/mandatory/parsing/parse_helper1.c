...
{
    if ((check_valid_path(texts->no) == -1 || check_valid_path(texts->so) == -1 || check_valid_path(texts->ea) == -1 || check_valid_path(texts->we) == -1)
        || (check_tformat(texts->no) == -1 || check_tformat(texts->so) == -1 || check_tformat(texts->ea) == -1 || check_tformat(texts->we) == -1) || (check_rgb(texts->c) == -1 || check_rgb(texts->f) == -1) || map->map_start == -1)
    {
